...
| Kind | Name | File | Description |
| --- | --- | --- | --- |
| Backend | vault-external | [gateway/backend.yaml](gateway/backend.yaml) | Backend that points to the Vault server as it can be reached from the browser. |
| BackendTLSPolicy | vault-external | [gateway/backend.yaml](gateway/backend.yaml) | Configures verification of the Vault server when the SecurityPolicy contacts the OIDC provider. |
